...
const TransactionsList = memo(function TransactionsList() {
  ...
  useEffect(() => {
    ...
    if (debouncedStartDate)
      params.startDate = new Date(debouncedStartDate).toISOString();
    if (debouncedEndDate)
      params.endDate = new Date(debouncedEndDate).toISOString();
  ...
  return (
    <section className="bg-slate-800 border border-slate-700 rounded-lg p-6">
      <h2 className="text-2xl font-semibold text-emerald-400 mb-4">
        Transactions{" "}
        {txnTotal > 0 && (
          <span className="text-sm text-slate-400">({txnTotal} total)</span>
        )}
      </h2>
              ...
              : txns.map((t) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2" style={{ height: "48px" }}>
                      <button
                        onClick={() =>
                          setSelectedTxnId(selectedTxnId === t.id ? null : t.id)
                        }
                        ...
                        title={
                          selectedTxnId === t.id ? "Hide Graph" : "Show Graph"
                        }
                      >
        ...
        {txnPageCount > 1 && (
          <>
            ...
            {(() => {
              ...
              let start = Math.max(1, txnPage - half);
              let end = Math.min(txnPageCount, start + windowSize - 1);
